...
def xr_import_NorESM(case, varlist, from_time, to_time, path=constants.get_input_datapath(),
    ...
    casename_nice = find_model_case_name.find_name(model, case)

    # Find files: returns a list of files to be read
    pathfile_list = filelist_NorESM(case, path, from_time, to_time, model, history_field=history_fld,
                                    comp=comp)
    ...
    if varlist is not None:
        ...
        drop_list = list((set(dummy.data_vars.keys()) - set(var_names_mod)) - set(constants.import_always_include))
        dummy.close()
    else:
        drop_list = None
    # Read the files:
    ds_out = xr.open_mfdataset(pathfile_list, decode_times=False, drop_variables=drop_list,
                               combine='nested', concat_dim='time', chunks=chunks)#, decode_cf=False )#parallel=True, )  # , autoclose=True)
    print(ds_out['time'])
...
def filelist_NorESM(case, path, from_time, to_time, model, history_field='.h0.', comp='atm'):
    """
    ...
    if model == 'NorESM':
        for var in varNames:
            ...
            if (var[-9:] =='_totLossR') or (var[-9:] =='_lifetime'):
                ...
                trac_n = var[:-9]
                addvars = [f'{trac_n}{lt}' for lt in lss_exts]# + [f'cb_{trac_n}']
                tot_lossR = _comp_total_lossR(addvars, trac_n, var, xr_ds)
                if var[-9:] =='_lifetime':
...
def _comp_total_lossR(addvars, trac_n, var, xr_ds):
